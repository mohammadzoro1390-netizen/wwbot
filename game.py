...
def ask_claude(question, user_context=""):
    """ارسال سوال به Groq (سازگار با OpenAI) - رایگان و بدون مشکل کلید"""
    import time
    ...
    try:
        ...
        print(f"🤖 Groq key[{key_idx+1}] daily:{_gemini_usage[key_idx]}")
        return answer.strip()
        ...
        if "429" in err:
            ...
            print(f"⚠️ Groq key[{key_idx+1}] rate limited, switching...")
            return ask_claude(question, user_context)

        print(f"⚠️ Groq error: {e}")
        return ("❌ هوش مصنوعی موقتاً در دسترس نیست.\n"
                "⚙️ کلید API را در config.py بررسی کنید.\n"
                "بقیه بخش‌های ربات (کشورها، خرید، پروفایل و...) به‌طور عادی کار می‌کنند.")
